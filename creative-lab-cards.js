...
function init() {
    ...
    let totalRows = 3;
    let gap = 16;
    let rowSize = document.querySelector(".twelfth-cards__list").offsetWidth / totalRows;
    let totalCols = Math.floor(document.querySelectorAll(".twelfth-card__item").length / totalRows);
    let containerHeight = document.querySelector(".twelfth-card__item").offsetHeight * totalCols + (gap * (totalRows - 1)) + "px";
    document.querySelector(".twelfth-cards__list").style.height = containerHeight;
    let colSize = document.querySelector(".twelfth-cards__list").offsetWidth / totalCols;
    ...
    for (let row = 0; row < totalRows; row++) {
        for (let col = 0; col < totalCols; col++) {
            cells.push({
                row: row,
                col: col,
                x: col * (document.querySelector(".twelfth-cards__list").offsetWidth / totalRows),
                y: row * ((document.querySelector(".twelfth-cards__list").offsetHeight + gap) / totalRows),
            });
        }
    }
    ...
    function init() {
        ...
        document.getElementById('fourteenth-cards__list').appendChild(renderer.domElement);
